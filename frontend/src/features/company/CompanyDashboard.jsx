    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0f766e 0%, #0e7490 100%)', padding: '48px 32px', color: '#fff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <h1 style={{ fontSize: '36px', fontWeight: '800', margin: '0 0 12px 0' }}>GD Topic Library</h1>
          <p style={{ fontSize: '16px', opacity: 0.85, maxWidth: '560px', margin: 0, lineHeight: 1.6 }}>
            Browse and shortlist discussion topics for your campus recruitment drives.
          </p>
          <p style={{ fontSize: '14px', opacity: 0.75, marginTop: '12px' }}>
            {shortlisted.size} topic{shortlisted.size !== 1 ? 's' : ''} shortlisted
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 32px 0' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '240px', display: 'flex', alignItems: 'center', background: '#fff', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '10px 16px', gap: '8px' }}>
            ...
            <input
              placeholder="Search topics for your GD rounds..."
              value={search}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '20px', paddingBottom: '40px' }}>
            {filteredTopics.map(topic => (
              <CompanyTopicCard
                key={topic.id}
                topic={topic}
                shortlisted={shortlisted.has(topic.id)}
                onToggleShortlist={() => toggleShortlist(topic.id)}
              />
            ))}
...
function CompanyTopicCard({ topic, shortlisted, onToggleShortlist }) {
  const tags = [
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '12px', border: shortlisted ? '2px solid #0f766e' : '1px solid #e5e7eb', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.07)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {tags.length > 0 && (
      ...
      <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: '#6b7280', flexWrap: 'wrap' }}>
        ...
        {topic.debate_balance && <span>⚖️ {topic.debate_balance}</span>}
        {topic.public_impact && <span>🌍 {topic.public_impact}</span>}
      </div>

      <button onClick={onToggleShortlist} style={{
        marginTop: '4px', padding: '10px', borderRadius: '8px', border: 'none',
        background: shortlisted ? '#dcfce7' : '#0f766e',
        color: shortlisted ? '#166534' : '#fff',
        fontWeight: '600', fontSize: '13px', cursor: 'pointer',
      }}>
        {shortlisted ? '✓ Shortlisted' : '🔖 Shortlist for Drive'}
      </button>
    </div>
  );
}